...
class Hero(Person):
    ...
    def levelUp(self, d=random.randint(1, 10)):
        self.file1 = open('hero_number.txt', 'w')
        if d:
            self.lvl += d
...
class Solder(Person):
    ...
    def folowToHero(self):
        self.file1 = open('hero_number.txt', 'r')
        self.line = self.file1.readline()
        if self.line != ",":
            print("Герой команды {} с ункальным номером {} взял в поход 1 солдата c уникальными id ({}) из команды {}.".format(self.team_id, self.line, self.id, self.team_id))
            self.file1.close()
teams = []
